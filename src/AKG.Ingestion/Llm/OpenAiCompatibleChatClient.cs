...
/// <summary>
/// <see cref="ILlmChatClient"/> backed by any OpenAI-compatible chat-completions API
/// (<c>/chat/completions</c>) — used for the OpenAI and Custom providers. Infrastructure adapter
/// (real HTTP); the response parsing is unit-tested. The API key comes from configuration.
/// </summary>
public sealed class OpenAiCompatibleChatClient : ILlmChatClient
{
    ...
    /// <summary>Initializes a new instance of the <see cref="OpenAiCompatibleChatClient"/> class.</summary>
    /// <param name="providerName">Provider label for diagnostics (e.g. "openai", "custom").</param>
    /// <param name="httpClientFactory">Factory used to create the HTTP client.</param>
    /// <param name="baseUrl">API base URL (e.g. <c>https://api.openai.com/v1</c>).</param>
    ...
    {
        using var client = _httpClientFactory.CreateClient($"{_providerName}-chat");
