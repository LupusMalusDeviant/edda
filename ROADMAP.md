...
|---|----------|-------|---------|-------------|--------------------|
| 0.1 | **Zero-Infra-Dev-Modus** `GRAPH_PROVIDER=memory` 🚧 | hoch | L (2–3 Sessions) | neuer `MemoryGraphDatabaseProvider` + `InMemoryCypherExecutor` (String-Dispatch der ~40 Cypher-Shapes); DI-Switch in `AddAkgServices` | **Stage 1–2/3 ✅ — bootet ohne Neo4j** (verifiziert: `GRAPH_PROVIDER=memory dotnet run` seedet WorldKnowledge/Rules/Tool-Domains, listening). Shapes: KnowledgeGraph, WorldKnowledge, Domains, Validator, ContextCompiler-Keyword-Pfad, GraphExpander. Offen: Stage 3 (Embedding-Cache/Head-Vektor/Entity-Shapes; No-ops unter `EMBEDDING_PROVIDER=null`) |
