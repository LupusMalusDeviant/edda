...
- [Nachtrag 2026-07-03: Design-Session-Issues (C2, C7, C9, E10, B5, A8, F16) auf Nutzer-Anweisung freigegeben — Bearbeitung in der Reihenfolge C7 → C9 → E10 → B5 → A8 → F16 → C2 (C2 zuletzt, hängt an der offenen C1-Threading-Rückfrage). Fortsetzung unterhalb des Abschluss-Markers.]
- C7 · blockiert · 2026-07-03 · Beschreibung passt nicht mehr: die MCP-Client-Infrastruktur samt Connector existiert vollständig — `McpKnowledgeConnector` (TypeId `mcp`, Thin-Config-Descriptor mit serverUrl/toolName/argsJson/label/domain + Secret-Token aus dem Credential-Store per ADR-0006), `McpKnowledgeSource` (SourceKind `mcp`, ruft via `IExternalMcpClientFactory`/`IExternalMcpClient` ein konfigurierbares Tool auf und mappt Text-Contents als verbundenen Subgraph: Source-Node + Items mit related-Links), Client-Stack (`ExternalMcpClient`, `HttpExternalMcpClientFactory`, Adapter) in src/AKG.Mcp/Client, DI in `McpServiceExtensions`; Tests: `McpKnowledgeConnectorTests` + `McpKnowledgeSourceTests`. Die Design-Fragen des Issues (welche Tools, welches Mapping) sind damit beantwortet (konfigurierbares Tool + generisches Text-Mapping). Kein Code geändert. Nächstes: C9.
- C9 · spec-erstellt · 2026-07-03 · docs/plans/0016: Kanten bekommen `validFrom`/`validUntil` analog zum C3-Node-Modell — beide Edge-Writer wechseln von Delete+Recreate auf temporales Replace (weggefallene Kanten schließen, Re-Deklaration re-opent mit first-seen validFrom; RuleLoader dabei auf die D1-UNWIND-Query + TimeProvider gehoben), `InvalidateSupersededRulesAsync` schließt die Kanten des invalidierten Fakts mit (außer eingehender SUPERSEDES), nur der Retrieval-Pfad (GraphExpander) filtert geschlossene Kanten; Anzeige/Diagnose zeigen Historie ungefiltert. Audit belegt: kein Leser nutzt Kanten-Properties → additiv sicher. Nächster Schritt: C9 umsetzen; danach E10.
